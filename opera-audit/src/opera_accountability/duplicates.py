...
def get_granules_from_grq(
    ...
    """Fetch granule IDs from a GRQ (OpenSearch) instance for duplicate detection.

    :param grq_url: GRQ OpenSearch URL (e.g. ``https://grq.example.com``).
    :param index: OpenSearch index pattern (e.g. ``grq_*_l3_dswx_hls-*``).
    :param product: Product key from ``config.yaml``.
    :param start: Optional start datetime for temporal filtering.
    :param end: Optional end datetime for temporal filtering.
    :param temporal: If ``True``, filter by ``starttime``; otherwise by
        ``creation_timestamp``.
    :param test_pattern: Optional compiled regex to filter native IDs.
    :returns: List of dicts with ``umm.GranuleUR`` structure compatible with
        :func:`detect_duplicates`.
    :raises ImportError: If ``opensearchpy`` is not installed.
    """
    try:
        import opensearchpy
        ...
        raise ImportError(
            "GRQ duplicate detection requires opensearchpy. "
            "Install with: pip install opensearch-py"
        ) from exc

    client = opensearchpy.OpenSearch(
        hosts=[grq_url],
        use_ssl=grq_url.startswith("https"),
        verify_certs=False,
        ssl_show_warn=False,
    )

    temporal_field = "starttime" if temporal else "creation_timestamp"
    temporal_query: dict[str, Any] = {"range": {temporal_field: {}}}

    if start is not None:
        temporal_query["range"][temporal_field]["gte"] = int(
            start.replace(tzinfo=timezone.utc).timestamp() * 1000
        )
    if end is not None:
        temporal_query["range"][temporal_field]["lte"] = int(
